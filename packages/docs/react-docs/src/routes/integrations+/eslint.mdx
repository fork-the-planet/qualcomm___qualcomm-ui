...
<IconButton icon={<CloseIcon />} aria-labelledby="close-label" />
```
...
Enforces that `Icon` components are either marked as decorative with `aria-hidden="true"` or have an accessible label.

Icons inside labeled parent components (like `IconButton` with `aria-label`) are automatically considered decorative and don't require additional attributes.

```jsx
// Invalid - no accessibility information
<Icon icon={SearchIcon} />

// Valid - decorative icon
<Icon icon={SearchIcon} aria-hidden="true" />

// Valid - meaningful icon with label
<Icon icon={WarningIcon} aria-label="Warning" />

// Valid - icon inside labeled parent
...
// Valid - aria-label
<TextInput placeholder="Enter name" aria-label="Full name" />

// Valid - compound component with Label
<TextInput.Root>
