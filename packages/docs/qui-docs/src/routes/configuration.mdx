...
- **omitted (default)** — uses [remix-flat-routes](https://github.com/kiliman/remix-flat-routes) conventions. Only folders ending in `+` create nested URL groups. Plain folders behave as standard React Router nested routes and do not contribute their own URL segment.
...
When routes are defined manually rather than auto-discovered, `routingStrategy` can also be a custom function. See [Custom Routing](/custom-routing) for the full setup.
